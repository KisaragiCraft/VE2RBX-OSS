...
<p align="center">
  <img src="assets/demo_full.gif" width="100%" alt="VE2RBX デモ - VoxEdit の作品を Roblox へ変換">
</p>
...
**2件まで同時変換、追加分はキューで待機。**
VE2RBX は最大2件を同時に変換し、それ以上は順番待ちにします。各ジョブは個別に停止でき、停止したジョブの途中出力だけを削除します。他の変換には影響しません。
...
ダウンロードは Windows 版を主導線とします。macOS 版の公開後は、Apple Silicon（`arm64`）用と Intel（`x86_64`）用の ZIP を別の macOS Release から取得できます。Blender は別途インストールが必要です。

macOS 版はアドホック署名のみで、**Apple の公証を受けていません**。初回起動時に macOS がブロックする場合は、一度アプリを開こうとした後、**システム設定 > プライバシーとセキュリティ** から **このまま開く** を選んでください。
